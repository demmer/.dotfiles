...
;; The version of the svn program.  We figure this out when our mode
;; gets started.
(defconst lvc-svn-svn-major-version nil)
(defconst lvc-svn-svn-minor-version nil)
(make-variable-buffer-local 'lvc-svn-major-svn-version)
(make-variable-buffer-local 'lvc-svn-minor-svn-version)
...
(defconst lvc-svn-status-dummy-line-pre-1-6 "       ")
(defconst lvc-svn-status-dummy-line         "        ")
(defconst lvc-svn-ustatus-dummy-line-pre-1-6 "                    ")
(defconst lvc-svn-ustatus-dummy-line         "                     ")

;; Dummy line appropriate for this buffer.
(defconst lvc-svn-dummy-line nil)
(make-variable-buffer-local 'lvc-svn-dummy-line)
...
(defconst lvc-svn-mode-map
  (let ((map (make-sparse-keymap 'lvc-svn-mode-map)))
    ...
    (define-key map "V" 'lvc-svn-resolve)
...
via `describe-key' on \\[describe-key]"
  ...
  (let ((ver (lvc-svn-determine-svn-version)))
    (setq lvc-svn-svn-major-version (car ver))
    (setq lvc-svn-svn-minor-version (cdr ver)))
  (if (>= (lvc-svn-svn-version-compare 1 6) 0)
      ;; 1.6 or newer
      (if (eq submode 'status)
	  (setq lvc-svn-linepat    lvc-svn-status-linepat
		lvc-svn-dummy-line lvc-svn-status-dummy-line)
	(setq lvc-svn-linepat    lvc-svn-ustatus-linepat
	      lvc-svn-dummy-line lvc-svn-ustatus-dummy-line))
    ;; pre 1.6
    (if (eq submode 'status)
	(setq lvc-svn-linepat    lvc-svn-status-linepat-pre-1-6
	      lvc-svn-dummy-line lvc-svn-status-dummy-line-pre-1-6)
      (setq lvc-svn-linepat    lvc-svn-ustatus-linepat-pre-1-6
	    lvc-svn-dummy-line lvc-svn-ustatus-dummy-line-pre-1-6)))
...
been locally modified\"."
  ...
  (if (eq last-command 'lvc-svn-explain-this-line)
    ...
    (let* ((state (lvc-svn-current-file-state))
      ...
      (setq res (concat res (format ", HEAD is %s" (lvc-svn-head-revision))))
      (message res))))
...
the file on this line."
  ...
  ;; externals or lvc-svn-head-revision will not be what we want.
  (interactive "P")
  (if (not (eq lvc-svn-submode 'ustatus))
...
(defun lvc-svn-log-head ()
  ...
  (let ((base (lvc-svn-current-file-base)))
    ...
    (if (and (numberp base)
	     (numberp (lvc-svn-head-revision))
	     (= base (lvc-svn-head-revision)))
...
disable the confirmation, you can set `lvc-resolve-confirm' to nil."
  (interactive "P")
  (let* ((files (lvc-svn-get-relevant-files arg))
	 (multiple-p (cdr files))
	 status
...
;	      (memq 'tree-conflicted state) ;not tested
	...
	(message "Resolve cancelled")
      (message "Resolving...")
      (setq status (lvc-svn-do-command-quietly "resolved" nil (mapcar 'car files)))
	  ...
	  (let ((cur files)
	    ...
	    (while cur
	      (setq pair (car cur))
	      (setq cur (cdr cur))
	      (lvc-svn-change-file-state (car pair) 'modified)))
	;; Otherwise an error happened, bitch appropriately
	(pop-to-buffer "*SVN-resolved*")
	(goto-char (point-min))
	(insert "\n"
		"*** The resolve was not completely successful.\n"
		"*** Check this buffer closely to determine what is wrong.\n"
		"\n")
	(error "Resolve, see *SVN-resolved* buffer for details.")))))
...
(defun lvc-svn-parse-state-string (str)
  ...
  (let (state)
	...
	;; pre 1.6
	;; 7th column - blank
	;; 8th column - out of date info (with -u, otherwise start of file name)
	(if (string-match "^.......\\*" str) (setq state (cons 'out-of-date
							       state)))
      ;; 1.6 or newer
      ;; 7th column - tree conflict
      (if (string-match "^......C" str) (setq state (cons 'tree-conflicted
							  state)))
      ;; 8th column - blank
      ;; 9th column - out of date info (with -u, otherwise start of
      ;; file name)
      (if (string-match "^........\\*" str) (setq state (cons 'out-of-date
							      state))))
    state))
...
(defun lvc-svn-head-revision ()
  ...
  (let ((markers lvc-svn-head-revision-markers)
	(head "HEAD") cur found)
...
(defun lvc-svn-top-head-revision ()
  ;; Return the HEAD revision for the top-level non-external
  ;; repository.  This will be the first element of our list.
  (cdr (car lvc-svn-head-revision-markers)))

(defun lvc-svn-determine-svn-version ()
  ;; Figure out the version of svn.
  ;; Call svn --version and return (major . minor) tuple.
  (let ((bufname "*SVN---version*")	
	status start)
    (setq status (lvc-svn-do-command-quietly "--version" '("-q")))
    (if (zerop status)
	(save-excursion
	  (set-buffer bufname)
	  (goto-char (point-min))
	  (if (looking-at "\\([0-9]+\\)\.\\([0-9]+\\)")
	    ...
	    (pop-to-buffer bufname)
	    (error "cannot parse svn --version output")))
      (pop-to-buffer bufname)
      (error "svn --version failed, see %s buffer for details" bufname))))

;; Returns -1,0,1 if the svn version is <,=,> maj.min
(defun lvc-svn-svn-version-compare (maj min)
  (cond ((> lvc-svn-svn-major-version maj) 1)
	((< lvc-svn-svn-major-version maj) -1)
	;; majors are equal, check minor
	(t (cond ((> lvc-svn-svn-minor-version min) 1)
		 ((< lvc-svn-svn-minor-version min) -1)
...
(defun lvc-svn-parse-output (beg)
  ...
  (let ((stuff-to-do t)
	...
	(setq lvc-svn-head-revision-markers
	      (nconc lvc-svn-head-revision-markers
