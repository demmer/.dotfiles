...
(append-no-dup "/cs/src/emacs-19/local/lisp" load-path)
...
;       (transient-mark-mode 1)
       (set-face-foreground 'primary-selection "ivory")
       (set-face-background 'primary-selection "royal blue")))
...
;; load in other files
;; (load-options-file (format "%s/.xemacs-options" *HOME*))
(load "modes")
(load "comment")
(load "added")
(load "mjdkeys")

;; don't load these at startup time - make things faster
(require 'complete)
;(require 'crypt++)
(require 'zwrite)
