...
;;; History of argument lists passed to tdb.
(defvar gud-tdb-history nil)

(defun gud-tdb-massage-args (file args)
  (cons "-fullname" args))

(defvar gud-tdb-marker-regexp
  (concat "\032\032\\([^" path-separator "\n]*\\)" path-separator
	  "\\([0-9]*\\)" path-separator ".*\n"))
...
(defun gud-tdb-marker-filter (string)
  (setq gud-marker-acc (concat gud-marker-acc string))
  (let ((output ""))

    ;; Process all the complete markers in this chunk.
    (while (string-match gud-tdb-marker-regexp gud-marker-acc)
      (setq
...
(defvar tdb-minibuffer-local-map nil
  "Keymap for minibuffer prompting of tdb startup command.")
(if tdb-minibuffer-local-map
    ()
  (setq tdb-minibuffer-local-map (copy-keymap minibuffer-local-map))
  (define-key
    tdb-minibuffer-local-map "\C-i" 'comint-dynamic-complete-filename))

;;;###autoload
(defun tdb (command-line)
  "Run tdb on program FILE in buffer *gud-FILE*.
The directory containing FILE becomes the initial working directory
and source-file directory for your debugger."
  (interactive
   (list (read-from-minibuffer "Run tdb (like this): "
			       (if (consp gud-tdb-history)
				   (car gud-tdb-history)
				 "tdb ")
			       tdb-minibuffer-local-map nil
			       '(gud-tdb-history . 1))))

  (gud-common-init command-line 'gud-tdb-massage-args
		   'gud-tdb-marker-filter 'gud-tdb-find-file)
  ...
  (local-set-key "\C-i" 'gud-tdb-complete-command)
  (local-set-key [menu-bar debug tbreak] '("Temporary Breakpoint" . gud-tbreak))
  (local-set-key [menu-bar debug finish] '("Finish Function" . gud-finish))
  (local-set-key [menu-bar debug up] '("Up Stack" . gud-up))
  (local-set-key [menu-bar debug down] '("Down Stack" . gud-down))
  ...
  (setq paragraph-start comint-prompt-regexp)
  (run-hooks 'tdb-mode-hook)
  )

;; One of the nice features of TDB is its impressive support for
;; context-sensitive command completion.  We preserve that feature
;; in the GUD buffer by using a TDB command designed just for Emacs.

;; The completion process filter indicates when it is finished.
(defvar gud-tdb-complete-in-progress)

;; Since output may arrive in fragments we accumulate partials strings here.
(defvar gud-tdb-complete-string)

;; We need to know how much of the completion to chop off.
(defvar gud-tdb-complete-break)

;; The completion list is constructed by the process filter.
(defvar gud-tdb-complete-list)

(defvar gud-comint-buffer nil)

(defun gud-tdb-complete-command ()
  "Perform completion on the TDB command preceding point.
This is implemented using the TDB `complete' command which isn't
available with older versions of TDB."
  (interactive)
  (let* ((end (point))
	 (command (save-excursion
		    (beginning-of-line)
    ...
    ;; Find the word break.  This match will always succeed.
    (string-match "\\(\\`\\| \\)\\([^ ]*\\)\\'" command)
    (setq gud-tdb-complete-break (match-beginning 2)
	  command-word (substring command gud-tdb-complete-break))
    ;; Temporarily install our filter function.
    (let ((gud-marker-filter 'gud-tdb-complete-filter))
      ;; Issue the command to TDB.
      (gud-basic-call (concat "complete " command))
      (setq gud-tdb-complete-in-progress t
	    gud-tdb-complete-string nil
	    gud-tdb-complete-list nil)
      ;; Slurp the output.
      (while gud-tdb-complete-in-progress
	(accept-process-output (get-buffer-process gud-comint-buffer))))
    ;; Protect against old versions of TDB.
    (and gud-tdb-complete-list
	 (string-match "^Undefined command: \"complete\""
		       (car gud-tdb-complete-list))
	 (error "This version of TDB doesn't support the `complete' command."))
    ;; Sort the list like readline.
    (setq gud-tdb-complete-list
	  (sort gud-tdb-complete-list (function string-lessp)))
    ;; Remove duplicates.
    (let ((first gud-tdb-complete-list)
	  (second (cdr gud-tdb-complete-list)))
      (while second
	(if (string-equal (car first) (car second))
	    (setcdr first (setq second (cdr second)))
	  (setq first second
    ...
    ;; Add a trailing single quote if there is a unique completion
    ;; and it contains an odd number of unquoted single quotes.
    (and (= (length gud-tdb-complete-list) 1)
	 (let ((str (car gud-tdb-complete-list))
	       (pos 0)
	       (count 0))
	   (while (string-match "\\([^'\\]\\|\\\\'\\)*'" str pos)
	     (setq count (1+ count)
		   pos (match-end 0)))
	   (and (= (mod count 2) 1)
		(setq gud-tdb-complete-list (list (concat str "'"))))))
    ;; Let comint handle the rest.
    (comint-dynamic-simple-complete command-word gud-tdb-complete-list)))
    
;; The completion process filter is installed temporarily to slurp the
;; output of TDB up to the next prompt and build the completion list.
(defun gud-tdb-complete-filter (string)
  (setq string (concat gud-tdb-complete-string string))
  (while (string-match "\n" string)
    (setq gud-tdb-complete-list
	  (cons (substring string gud-tdb-complete-break (match-beginning 0))
		gud-tdb-complete-list))
    (setq string (substring string (match-end 0))))
  (if (string-match comint-prompt-regexp string)
      (progn
	(setq gud-tdb-complete-in-progress nil)
	string)
    (progn
      (setq gud-tdb-complete-string string)
      "")))
